...
*/
    ...
    foreach ($items as $item)
    {
        ...
        if (pnConfigGetVar('shorturls') && pnConfigGetVar('shorturlstype') == 0) {
            $options[] = array('url'   => pnModURL('News', 'user', 'display', array('sid' => $item['sid'], 'from' => $item['from'], 'urltitle' => $item['urltitle'])),
...
function News_admin_modifyconfig()
{
    ...
    if ($modvars['picupload_uploaddir'] == 'images/news_picupload' && !is_dir($modvars['picupload_uploaddir'])) {
        $render->assign('picupload_createfolder', true);
    } else {
        $render->assign('picupload_createfolder', false);
    }
    ...
    if ($modvars['picupload_enabled']) {
        if ($createfolder && !empty($modvars['picupload_uploaddir'])) {
            if (is_dir($modvars['picupload_uploaddir'])) {
                LogUtil::registerError(__f('Warning! The image upload directory at [%s] already exists. Make sure that this folder is accessible via the web and writable by the webserver.', $modvars['picupload_uploaddir'], $dom));
            } else {
                ...
                if (FileUtil::mkdirs($modvars['picupload_uploaddir'], 0777)) {
                    ...
                    if (FileUtil::writeFile($modvars['picupload_uploaddir'] . '/.htaccess', $htaccessContent)) {
                        LogUtil::registerStatus(__f('News publisher created the image upload directory successfully at [%s] and wrote an .htaccess file there for security. Make sure that this folder is accessible via the web and writable by the webserver.', $modvars['picupload_uploaddir'], $dom));
                    } else {
                        LogUtil::registerStatus(__f('News publisher created the image upload directory successfully at [%s], but could not write the .htaccess file there. Make sure that this folder is accessible via the web and writable by the webserver.', $modvars['picupload_uploaddir'], $dom));
                    }
                } else {
                    LogUtil::registerStatus(__f('Warning! News publisher could not create the specified image upload directory [%s]. Try to create it yourself and make sure that this folder is accessible via the web and writable by the webserver.', $modvars['picupload_uploaddir'], $dom));
                }
            }
        }
    }
