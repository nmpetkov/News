 ...
 * @version    $Id: decodeurl.php 433 2010-08-22 17:38:43Z espaan $
...
function News_userapi_decodeurl($args)
{
    ...
    if ($func == 'display' || $func == 'displaypdf') {
        ...
        if (isset($permalinkkeys['%articleid%']) && isset($args['vars'][$permalinkkeys['%articleid%']]) && is_numeric($args['vars'][$permalinkkeys['%articleid%']])) {
            pnQueryStringSetVar('sid', $args['vars'][$permalinkkeys['%articleid%']]);
            $nextvar = $permalinkkeys['%articleid%']+1;
        } else {
            pnQueryStringSetVar('title', $args['vars'][$permalinkkeys['%articletitle%']]);
