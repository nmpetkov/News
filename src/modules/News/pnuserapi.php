...
class news_result_checker
{
    ...
    var $enablecategorybasedpermissions;

    function news_result_checker($enablecategorization, $enablecategorybasedpermissions)
    {
...
function News_userapi_getall($args)
{
    ...
    // or filter articles before the current date
    ...
    $permChecker = new news_result_checker($modvars['enablecategorization'], $modvars['enablecategorybasedpermissions']);
    ...
    if ($modvars['enablecategorization'] && $objArray && isset($args['catregistry']) && $args['catregistry']) {
...
function News_userapi_getArticleLinks($info)
{
    ...
    if (pnModAvailable('EZComments') &&  pnModIsHooked('EZComments', 'News') && $info['disallowcomments'] == 0) {
        if ($shorturls && $shorturlstype == 0) {
            $comment = DataUtil::formatForDisplay(pnModURL('News', 'user', 'display', array('sid' => $info['sid'], 'from' => $info['from'], 'urltitle' => $info['urltitle'], '__CATEGORIES__' => $info['categories']), null, 'comments'));
        } else {
    ...
    if (SecurityUtil::checkPermission('News::', "$info[cr_uid]::$info[sid]", ACCESS_READ)) {
        if ($shorturls && $shorturlstype == 0) {
            $fullarticle = DataUtil::formatForDisplay(pnModURL('News', 'user', 'display', array('sid' => $info['sid'], 'from' => $info['from'], 'urltitle' => $info['urltitle'], '__CATEGORIES__' => $info['categories'])));
        } else {
    ...
    if ($shorturls && $shorturlstype == 0) {
        $links = array ('category'        => DataUtil::formatForDisplay(pnModURL('News', 'user', 'view', array('prop' => 'Main', 'cat' => $info['catvar']))),
                        ...
                        'permalink'       => DataUtil::formatForDisplayHTML(pnModURL('News', 'user', 'display', array('sid' => $info['sid'], 'from' => $info['from'], 'urltitle' => $info['urltitle'], '__CATEGORIES__' => $info['categories']), null, null, true)),
                        'comment'         => $comment,
                        'fullarticle'     => $fullarticle,
                        'searchtopic'     => $searchtopic,
                        'print'           => DataUtil::formatForDisplay(pnModURL('News', 'user', 'display', array('sid' => $info['sid'], 'from' => $info['from'], 'urltitle' => $info['urltitle'], '__CATEGORIES__' => $info['categories'], 'theme' => 'Printer'))),
